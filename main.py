...
# config.gpu_options.per_process_gpu_memory_fraction = 0.52 # 占用GPU90%的显存
session = tf.Session(config=config)

from CSDVAE import CSDVAE
from DAE import DAE
...
parser.add_argument('--model_name', choices=['CSDVAE'], default='CSDVAE')
parser.add_argument('--data_name', choices=['politic_old','politic_new','citeulike_a','citeulike_t'], default='citeulike_a')
parser.add_argument('--test_fold', type=int, default=100)
...
parser.add_argument('--batch_normalization', choices=['True','False'], default = 'True')

parser.add_argument('--hidden_neuron', type=int, default=200)
...
parser.add_argument('--g_act', choices=['Sigmoid','Relu','Elu','Tanh',"Identity"], default = 'Elu') # Decoder Activation
...
parser.add_argument('--lambda_w',type=float , default =1) # SDAE weight std. / weight , bias regularization / cost1
parser.add_argument('--lambda_n',type=float , default = 0.0001) # SDAE output (cost2)
...
elif data_name=='citeulike_a':#数据稀疏性更大！
    num_users=5551
    num_items = 16981
    num_total_ratings =204986
    num_voca=8000

elif data_name=='citeulike_t':#数据稀疏性更大！
    num_users=7947
    num_items = 25975
    num_total_ratings =134860
    num_voca=20000
...
with tf.Session() as sess:
    if model_name == "CSDVAE":
        lambda_u = args.lambda_u
        lambda_v = args.lambda_v
        lambda_w = args.lambda_w
        lambda_n = args.lambda_n
        lambda_x = args.lambda_x
        lambda_list = [lambda_u, lambda_w, lambda_v, lambda_n,lambda_x]
        corruption_level = args.corruption_level

        layer_structure1 = [num_voca,3250, 2 * hidden_neuron]  # encoder
        layer_structure2 = [hidden_neuron,3250, num_voca]  # decoder
        n_layer = len(layer_structure1) + len(layer_structure2) - 1
        # 定义为了字典类型，字典类型的key error相当于列表的越界错误
        pre_W = dict()
        pre_b = dict()

        for itr in range(n_layer - 1):
            if itr < int(n_layer / 2):
                initial_DAE = DAE(layer_structure1[itr], layer_structure1[itr + 1], num_items, num_voca, itr, "sigmoid")
                pre_W[itr], pre_b[itr] = initial_DAE.do_not_pretrain()
                # version2
                # pre_W[iter]=initial_DAE.cnn_weight_Variable(shape=[5,1,layer_structure1[itr], layer_structure1[itr + 1]])
                # pre_b[itr]=initial_DAE.cnn_bias_Variable(shape=[layer_structure1[itr+1]])
            else:
                initial_DAE = DAE(layer_structure2[itr - int(n_layer / 2)],
                                  layer_structure2[itr + 1 - int(n_layer / 2)], num_items, num_voca, itr, "sigmoid")
                pre_W[itr], pre_b[itr] = initial_DAE.do_not_pretrain()

        model = CSDVAE(sess, num_users, num_items, num_voca, hidden_neuron,current_time,
                  batch_size, lambda_list, layer_structure1,layer_structure2, train_epoch,
                  pre_W, pre_b, f_act,g_act,
                  corruption_level, keep_prob,num_train_true,num_test_true,
