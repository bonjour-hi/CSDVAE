...
class CSDVAE():
    def __init__(self, sess, num_user, num_item, num_voca, hidden_neuron,current_time,
               batch_size, lambda_list,layer_structure1,layer_structure2, train_epoch,
               pre_W, pre_b, f_act,g_act,
               cdl_corruption_level, cdl_keep_prob,num_train_true,num_test_true,
        ...
        self.lambda_x = lambda_list[4]

        self.layer_structure1 = layer_structure1
        self.layer_structure2 = layer_structure2
        ...
        self.num_train_true = num_train_true
        self.num_test_true = num_test_true
        ...
        self.lr = tf.train.exponential_decay(cdl_learning_rate, self.step, self.decay_step, self.decay_rate,  name="lr",
                                   staircase=True)

        # self.lr = tf.train.polynomial_decay(
        #     cdl_learning_rate, self.step, self.decay_step, self.decay_rate,  name="lr"
        # )
    ...
    def prepare_model(self):
        ...
        X_corrupted = tf.multiply(self.model_X,self.model_mask_corruption)
        real_batch_size = tf.cast(tf.shape(self.model_X)[0], tf.int32)
        mean, stddev, self.Encoded_X = SDAE_encode(self.model_name, X_corrupted, self.layer_structure1, self.Weight,
                                              self.bias,
                                              self.do_batch_norm, self.f_act, self.model_keep_prob)

        layer_num = len(self.layer_structure2) + len(self.layer_structure1) - 1
        sdae_output = SDAE_decode(self.model_name, self.Encoded_X, layer_num, self.layer_structure2, self.Weight, self.bias,
                                  self.do_batch_norm, self.g_act, self.model_keep_prob)

        with tf.variable_scope("CSDVAE_Variable"):
        ...
        loss_2 = self.lambda_v * tf.nn.l2_loss((batch_v_jk - self.Encoded_X))

        sdae_output = tf.clip_by_value(sdae_output, 1e-8, 1)
        ae=tf.clip_by_value(1-sdae_output, 1e-8, 1)

        marginal_likelihood = tf.reduce_sum(
            self.model_X * tf.log(sdae_output) + (1 - self.model_X) * tf.log(ae), 1)

        KL_divergence = 0.5 * tf.reduce_sum(tf.square(mean) + tf.square(stddev) - tf.log(1e-8 + tf.square(stddev)) - 1,
                                            1)
        ...
        loss_3 = self.lambda_n *(-ELBO)

        mul = tf.matmul(self.u_ik, batch_v_jk, transpose_b=True)
        loss_4 = tf.reduce_sum(tf.multiply(self.model_C,
                                           tf.square(self.model_input_R - mul)))
        ...
        for var in tf.trainable_variables():
            if ("CSDVAE_Variable" in var.name):
    ...
    def run(self):
        ...
        make_records(self.result_path, self.test_acc_list, self.test_precision_list, self.test_recall_list,
                     self.test_recallAtm_list,self.test_Map_list,self.map_final_list, self.current_time,
                     self.args, self.model_name, self.data_name, self.train_ratio, self.hidden_neuron, self.random_seed,
                     self.optimizer_method, self.lr)
    ...
    def test_model(self, itr):
        ...
        RECALL = evaluation(self.test_R, self.test_mask_R,self.Estimated_R,
                                                                          self.num_test_ratings, self.num_test_true)
        RECALLat300=recall_M(self.num_item,self.num_user,300,self.test_R,self.Estimated_R,self.num_test_ratings)


        Map=map(self.num_item,self.num_user,self.test_R,self.Estimated_R)

        self.test_recall_list.append(RECALL)
        self.test_recallAtm_list.append(RECALLat300)
        self.test_Map_list.append(Map)

        if itr % self.display_step == 0:
            print("Testing //", "Epoch %d //" % (itr), " Total cost = {:.2f}".format(Cost),
                  "Elapsed time : %d sec" % (time.time() - start_time))
            print("RECALL = {:.8f}".format(RECALL),"Map = {:.8f}".format(Map), "RECALLat300 = {:.10f}".format(RECALLat300))

            print("=" * 100)

        if RECALLat300 >= self.max_recall_at_300:
            self.max_recall_at_300 = RECALLat300
            self.min_epoch = itr
            self.patience = 0
        else:
            self.patience = self.patience + 1

        if (itr > 500) and (self.patience >= self.total_patience):
            self.test_recall_list.append(self.test_recall_list[self.min_epoch])
            self.test_recallAtm_list.append(self.test_recallAtm_list[self.min_epoch])

            for i in range(50,300,50):
                RECALLat=recall_M(self.num_item, self.num_user, i, self.test_R, self.Estimated_R,
                         self.num_test_ratings)
                self.map_final_list.append(RECALLat)
